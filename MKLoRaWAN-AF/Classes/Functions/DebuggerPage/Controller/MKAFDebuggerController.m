...
@interface MKAFDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_af_centralManagerLogDelegate,
MKAFDebuggerCellDelegate>
...
@property (nonatomic, strong)MKAFDebuggerButton *deleteButton;

@property (nonatomic, strong)MKAFDebuggerButton *exportButton;
...
@implementation MKAFDebuggerController

- (void)dealloc {
    NSLog(@"MKAFDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_af_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self loadSubViews];
    [MKAFCentralManager shared].logDelegate = self;
    [self readDataFromLocal];
    //进入debugger模式之后，即使设备断开连接，也要停留在当前页面
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_af_startDebuggerMode" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceDisconnect)
                                                 name:mk_af_peripheralConnectStateChangedNotification
                                               object:nil];
}

#pragma mark - super method
- (void)leftButtonMethod {
    if ([MKAFCentralManager shared].connectStatus != mk_af_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
    ...
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKAFCentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKAFDebuggerCell *cell = [MKAFDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - mk_af_centralManagerLogDelegate
...
#pragma mark - MKAFDebuggerCellDelegate
- (void)af_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    if (self.isLoading) {
        //正在加载，不允许操作
        [self.tableView reloadData];
        return;
    }
    MKAFDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_af_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_export_disableIcon.png");
    [self.startButton setTitle:@"Stop" forState:UIControlStateNormal];
    [self.contentList removeAllObjects];
    [[MKAFCentralManager shared] notifyLogData:YES];
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_af_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKAFLogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKAFDebuggerCellModel *cellModel = self.dataList[i];
...
- (void)readDataFromLocal {
    ...
    [MKAFLogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)loadTableViewDatas:(NSArray *)logList {
    ...
    for (NSInteger i = 0; i < logList.count; i ++) {
        ...
        MKAFDebuggerCellModel *cellModel = [[MKAFDebuggerCellModel alloc] init];
...
- (void)saveDataListToLocal {
    ...
    [MKAFLogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKAFDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_delete_disableIcon.png");
...
- (void)showTipsAlert:(NSString *)msg {
    ...
    [alertView showAlertWithTitle:@"Tips!" message:msg notificationName:@"mk_af_needDismissAlert"];
}
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKAFDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKAFDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_delete_disableIcon.png");
...
- (MKAFDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKAFDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFDebuggerController", @"af_export_disableIcon.png");
