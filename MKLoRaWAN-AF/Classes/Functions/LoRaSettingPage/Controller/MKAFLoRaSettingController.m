...
@interface MKAFLoRaSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFLoRaSettingModel *dataModel;

@end

@implementation MKAFLoRaSettingController

- (void)dealloc {
    NSLog(@"MKAFLoRaSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFLoRaSettingController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFLoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFLoRaSettingModel alloc] init];
    }
    return _dataModel;
}
