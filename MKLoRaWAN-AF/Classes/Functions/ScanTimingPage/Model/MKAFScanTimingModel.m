...
@implementation MKAFScanTimingModel
...
- (BOOL)readScanTimePoint {
    ...
    [MKAFInterface af_readScanAlwaysOnTimingReportTimePointWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configScanTimePoint:(NSArray <MKAFScanTimePointModel *>*)pointList {
    __block BOOL success = NO;
    [MKAFInterface af_configScanAlwaysOnTimingReportTimePoint:pointList sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
