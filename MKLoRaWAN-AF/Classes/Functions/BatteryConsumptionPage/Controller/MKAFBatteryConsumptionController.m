//
//  MKAFBatteryConsumptionController.m
...
@interface MKAFBatteryConsumptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFBatteryConsumptionModel *dataModel;

@end

@implementation MKAFBatteryConsumptionController

- (void)dealloc {
    NSLog(@"MKAFBatteryConsumptionController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        MKAFBatteryInfoCell *cell = [MKAFBatteryInfoCell initCellWithTableView:tableView];
...
- (void)batteryReset {
    ...
    alertView.notificationName = @"mk_af_needDismissAlert";
    @weakify(self);
...
- (void)sendBatteryResetCommandToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKAFInterface af_batteryResetWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)loadSection0Datas {
    MKAFBatteryInfoCellModel *cellModel = [[MKAFBatteryInfoCellModel alloc] init];
...
- (MKAFBatteryConsumptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFBatteryConsumptionModel alloc] init];
    }
    return _dataModel;
}
