...
@implementation MKAFFilterRelationshipCellModel
@end

@interface MKAFFilterRelationshipCell ()
...
@implementation MKAFFilterRelationshipCell

+ (MKAFFilterRelationshipCell *)initCellWithTableView:(UITableView *)tableView {
    MKAFFilterRelationshipCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAFFilterRelationshipCellIdenty"];
    if (!cell) {
        cell = [[MKAFFilterRelationshipCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@""];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(af_filterRelationshipChanged:dataListIndex:)]) {
            [self.delegate af_filterRelationshipChanged:self.dataModel.index dataListIndex:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKAFFilterRelationshipCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAFFilterRelationshipCellModel.class]) {
        return;
    }
