...
@interface MKAFFilterByMacModel ()
...
@implementation MKAFFilterByMacModel
...
- (BOOL)readFilterPreciseMatch {
    ...
    [MKAFInterface af_readFilterByMacPreciseMatchWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterPreciseMatch {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByMacPreciseMatch:self.match sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReverseFilter {
    __block BOOL success = NO;
    [MKAFInterface af_readFilterByMacReverseFilterWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReverseFilter {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByMacReverseFilter:self.filter sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readMacList {
    __block BOOL success = NO;
    [MKAFInterface af_readFilterMACAddressListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMacList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterMACAddressList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
