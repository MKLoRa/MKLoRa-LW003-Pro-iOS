...
@interface MKAFBXPTagContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFBXPTagContentModel *dataModel;

@end

@implementation MKAFBXPTagContentController

- (void)dealloc {
    NSLog(@"MKAFBXPTagContentController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BXP-T&S Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFBXPTagContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFBXPTagContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFBXPTagContentModel alloc] init];
    }
    return _dataModel;
}
