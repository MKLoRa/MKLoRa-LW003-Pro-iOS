...
@interface MKAFBeaconContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFBeaconContentModel *dataModel;

@end

@implementation MKAFBeaconContentController

- (void)dealloc {
    NSLog(@"MKAFBeaconContentController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFBeaconContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFBeaconContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFBeaconContentModel alloc] init];
    }
    return _dataModel;
}
