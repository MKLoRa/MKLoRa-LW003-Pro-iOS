...
@interface MKAFTLMContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFTLMContentModel *dataModel;

@end

@implementation MKAFTLMContentController

- (void)dealloc {
    NSLog(@"MKAFTLMContentController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFTLMContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFTLMContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFTLMContentModel alloc] init];
    }
    return _dataModel;
}
