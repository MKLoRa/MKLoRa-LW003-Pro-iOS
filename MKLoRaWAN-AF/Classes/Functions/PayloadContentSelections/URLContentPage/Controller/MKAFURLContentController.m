...
@interface MKAFURLContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFURLContentModel *dataModel;

@end

@implementation MKAFURLContentController

- (void)dealloc {
    NSLog(@"MKAFURLContentController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFURLContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFURLContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFURLContentModel alloc] init];
    }
    return _dataModel;
}
