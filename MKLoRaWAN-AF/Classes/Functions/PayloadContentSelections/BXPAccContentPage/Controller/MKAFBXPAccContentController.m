...
@interface MKAFBXPAccContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFBXPAccContentModel *dataModel;

@end

@implementation MKAFBXPAccContentController

- (void)dealloc {
    NSLog(@"MKAFBXPAccContentController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFBXPAccContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFBXPAccContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFBXPAccContentModel alloc] init];
    }
    return _dataModel;
}
