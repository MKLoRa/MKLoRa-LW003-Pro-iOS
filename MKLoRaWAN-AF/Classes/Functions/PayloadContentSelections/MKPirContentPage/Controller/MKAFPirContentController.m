...
@interface MKAFPirContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFPirContentModel *dataModel;

@end

@implementation MKAFPirContentController

- (void)dealloc {
    NSLog(@"MKAFPirContentController销毁");
}
...
- (void)mk_textSwitchCellStatusChanged:(BOOL)isOn index:(NSInteger)index {
    ...
    if (index == 3) {
        //Pir Delay Response Status
        self.dataModel.pirDelayResponse = isOn;
    ...
    if (index == 6) {
        ...
        self.dataModel.sensorDetection = isOn;
...
- (void)loadSection1Datas {
    MKTextSwitchCellModel *cellModel1 = [[MKTextSwitchCellModel alloc] init];
    cellModel1.index = 3;
    cellModel1.msg = @"PIR Delay Response Status";
    cellModel1.isOn = self.dataModel.pirDelayResponse;
    ...
    cellModel4.isOn = self.dataModel.sensorDetection;
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"MK-PIR Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFPirContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFPirContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFPirContentModel alloc] init];
    }
    return _dataModel;
}
