//
//  MKAFBXPInfoContentController.m
...
@property (nonatomic, strong)MKAFBXPInfoContentModel *dataModel;

@end

@implementation MKAFBXPInfoContentController

- (void)dealloc {
    NSLog(@"MKAFBXPInfoContentController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFBXPInfoContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFBXPInfoContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFBXPInfoContentModel alloc] init];
    }
    return _dataModel;
}
