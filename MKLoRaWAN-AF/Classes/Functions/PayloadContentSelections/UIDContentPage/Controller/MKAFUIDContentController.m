...
@interface MKAFUIDContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFUIDContentModel *dataModel;

@end

@implementation MKAFUIDContentController

- (void)dealloc {
    NSLog(@"MKAFUIDContentController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFUIDContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFUIDContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFUIDContentModel alloc] init];
    }
    return _dataModel;
}
