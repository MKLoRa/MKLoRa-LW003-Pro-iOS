//
//  MKAFBXPBeaconContentController.m
...
@interface MKAFBXPBeaconContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFBXPBeaconContentModel *dataModel;

@end

@implementation MKAFBXPBeaconContentController

- (void)dealloc {
    NSLog(@"MKAFBXPBeaconContentController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFBXPBeaconContentController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFBXPBeaconContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFBXPBeaconContentModel alloc] init];
    }
    return _dataModel;
}
