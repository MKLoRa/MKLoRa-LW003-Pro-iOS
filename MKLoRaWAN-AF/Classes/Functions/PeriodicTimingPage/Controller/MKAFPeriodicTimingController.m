//
//  MKAFPeriodicTimingController.m
...
@interface MKAFPeriodicTimingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFPeriodicTimingModel *dataModel;

@end

@implementation MKAFPeriodicTimingController

- (void)dealloc {
    NSLog(@"MKAFPeriodicTimingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFPeriodicTimingController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFPeriodicTimingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFPeriodicTimingModel alloc] init];
    }
    return _dataModel;
}
