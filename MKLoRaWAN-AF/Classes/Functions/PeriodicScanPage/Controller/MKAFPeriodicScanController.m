...
@interface MKAFPeriodicScanController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFPeriodicScanModel *dataModel;

@end

@implementation MKAFPeriodicScanController

- (void)dealloc {
    NSLog(@"MKAFPeriodicScanController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFPeriodicScanController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFPeriodicScanModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFPeriodicScanModel alloc] init];
    }
    return _dataModel;
}
