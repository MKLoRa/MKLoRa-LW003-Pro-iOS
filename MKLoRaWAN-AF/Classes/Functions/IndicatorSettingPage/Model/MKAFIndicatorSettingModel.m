...
@interface MKAFIndicatorSettingModel ()
...
@implementation MKAFIndicatorSettingModel
...
- (BOOL)readIndicatorSettings {
    ...
    [MKAFInterface af_readIndicatorSettingsWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIndicatorSettings {
    __block BOOL success = NO;
    [MKAFInterface af_configIndicatorSettings:self.lowPower charged:self.charged broadcast:self.broadcast sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
