...
static NSString *const localPasswordKey = @"mk_af_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKAFScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_af_centralManagerScanDelegate,
MKAFScanPageCellDelegate,
MKSearchButtonDelegate,
MKAFTabBarControllerDelegate>
...
@implementation MKAFScanController

- (void)dealloc {
    NSLog(@"MKAFScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKAFCentralManager shared] stopScan];
    [MKAFCentralManager removeFromCentralList];
}
...
- (void)viewDidLoad {
    ...
    [MKAFCentralManager shared].delegate = self;
...
- (void)rightButtonMethod {
    UIViewController *vc = [[CTMediator sharedInstance] CTMediator_LORAWAN_AF_AboutPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKAFScanPageCell *cell = [MKAFScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_af_centralManagerScanDelegate
- (void)mk_af_receiveDevice:(NSDictionary *)deviceModel {
    MKAFScanPageModel *model = [MKAFScanPageModel mk_modelWithJSON:deviceModel];
...
#pragma mark - MKAFScanPageCellDelegate
- (void)af_scanCellConnectButtonPressed:(NSInteger)index {
    [self connectDeviceWithModel:self.dataList[index]];
}

#pragma mark - MKAFTabBarControllerDelegate
- (void)mk_af_needResetScanDelegate:(BOOL)need {
    if (need) {
        [MKAFCentralManager shared].delegate = self;
    }
...
- (void)refreshButtonPressed {
    if ([MKAFCentralManager shared].centralStatus != mk_af_centralManagerStatusEnable) {
...
- (void)scanTimerRun{
    ...
    [[MKAFCentralManager shared] startScan];
...
- (void)updateDataWithScanModel:(MKAFScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKAFScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKAFScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKAFScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKAFScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKAFScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKAFScanPageModel *dataModel = self.dataList[currentIndex];
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKAFScanPageModel *)scanDataModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKAFCentralManager shared] stopScan];
    if (self.scanTimer) {
    ...
    [alertView showAlertWithTitle:@"Enter password" message:msg notificationName:@"mk_af_needDismissAlert"];
}

- (void)connectDeviceWithDataModel:(MKAFScanPageModel *)scanDataModel {
    ...
    [[MKAFConnectModel shared] connectDevice:scanDataModel.peripheral password:(scanDataModel.needPassword ? password : @"") deviceType:scanDataModel.deviceType sucBlock:^{
...
- (void)configParams {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"af_readRecordDataDayNumKey"];
    //读取设备存储的扫描信息页面，显示的设备总共存储了多少条的扫描数据
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"af_recordDataTotalSumKey"];
    
    [MKAFDatabaseManager clearDataTable];
    [MKAFDatabaseManager initDataBase];
}

- (void)pushTabBarPage {
    MKAFTabBarController *vc = [[MKAFTabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFScanController", @"af_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFScanController", @"af_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
