...
@interface MKAFTabBarController ()
...
@implementation MKAFTabBarController

- (void)dealloc {
    NSLog(@"MKAFTabBarController销毁");
...
- (void)viewDidDisappear:(BOOL)animated{
    ...
    if (![[self.navigationController viewControllers] containsObject:self]){
        [[MKAFCentralManager shared] disconnect];
    }
}
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(gotoScanPage)
                                                 name:@"mk_af_popToRootViewControllerNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(dfuUpdateComplete)
                                                 name:@"mk_af_centralDeallocNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(centralManagerStateChanged)
                                                 name:mk_af_centralManagerStateChangedNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(disconnectTypeNotification:)
                                                 name:mk_af_deviceDisconnectTypeNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceEnterDebuggerMode)
                                                 name:@"mk_af_startDebuggerMode"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStopDebuggerMode)
                                                 name:@"mk_af_stopDebuggerMode"
                                               object:nil];
}
...
- (void)gotoScanPage {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_af_needResetScanDelegate:)]) {
            [self.delegate mk_af_needResetScanDelegate:NO];
        }
    }];
}
...
- (void)dfuUpdateComplete {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_af_needResetScanDelegate:)]) {
            [self.delegate mk_af_needResetScanDelegate:YES];
        }
    }];
}
...
- (void)showAlertWithMsg:(NSString *)msg title:(NSString *)title{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_af_needDismissAlert" object:nil];
    //让所有MKPickView消失
    ...
    [alertView showAlertWithTitle:title message:msg notificationName:@"mk_af_needDismissAlert"];
}

- (void)loadSubPages {
    MKAFLoRaController *loraPage = [[MKAFLoRaController alloc] init];
    loraPage.tabBarItem.title = @"LORA";
    loraPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_lora_tabBarUnselected.png");
    loraPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_lora_tabBarSelected.png");
    MKBaseNavigationController *advNav = [[MKBaseNavigationController alloc] initWithRootViewController:loraPage];

    MKAFBleGatewayController *scannerPage = [[MKAFBleGatewayController alloc] init];
    scannerPage.tabBarItem.title = @"SCANNER";
    scannerPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_scanner_tabBarUnselected.png");
    scannerPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_scanner_tabBarSelected.png");
    MKBaseNavigationController *scannerNav = [[MKBaseNavigationController alloc] initWithRootViewController:scannerPage];

    MKAFGeneralController *settingPage = [[MKAFGeneralController alloc] init];
    settingPage.tabBarItem.title = @"GENERAL";
    settingPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_setting_tabBarUnselected.png");
    settingPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_setting_tabBarSelected.png");
    MKBaseNavigationController *settingNav = [[MKBaseNavigationController alloc] initWithRootViewController:settingPage];
    
    MKAFDeviceSettingController *deviceInfo = [[MKAFDeviceSettingController alloc] init];
    deviceInfo.tabBarItem.title = @"DEVICE";
    deviceInfo.tabBarItem.image = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_device_tabBarUnselected.png");
    deviceInfo.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-AF", @"MKAFTabBarController", @"af_device_tabBarSelected.png");
