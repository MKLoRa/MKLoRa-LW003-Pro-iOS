...
@interface MKAFLoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKAFLoRaPageModel *dataModel;

@end

@implementation MKAFLoRaController

- (void)dealloc {
    NSLog(@"MKAFLoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKAFLoRaSettingController *vc = [[MKAFLoRaSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.row == 2) {
        //Application Settings
        MKAFLoRaAppSettingController *vc = [[MKAFLoRaAppSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (MKAFLoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFLoRaPageModel alloc] init];
    }
    return _dataModel;
}
