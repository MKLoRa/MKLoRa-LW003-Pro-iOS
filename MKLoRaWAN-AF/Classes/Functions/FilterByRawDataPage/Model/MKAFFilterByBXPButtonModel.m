...
@interface MKAFFilterByBXPButtonModel ()
...
@implementation MKAFFilterByBXPButtonModel
...
- (BOOL)readFilterStatus {
    ...
    [MKAFInterface af_readBXPButtonFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByBXPButtonStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readButtonAlarmContent {
    __block BOOL success = NO;
    [MKAFInterface af_readBXPButtonAlarmFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configBXPButtonContent {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByBXPButtonAlarmStatus:self.singlePress
