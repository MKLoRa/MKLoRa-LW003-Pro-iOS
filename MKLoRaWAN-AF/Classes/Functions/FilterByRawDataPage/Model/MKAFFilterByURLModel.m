...
@interface MKAFFilterByURLModel ()
...
@implementation MKAFFilterByURLModel
...
- (BOOL)readFilterStatus {
    ...
    [MKAFInterface af_readFilterByURLStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByURLStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readURLContent {
    __block BOOL success = NO;
    [MKAFInterface af_readFilterByURLContentWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configURLContent {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByURLContent:self.url sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
