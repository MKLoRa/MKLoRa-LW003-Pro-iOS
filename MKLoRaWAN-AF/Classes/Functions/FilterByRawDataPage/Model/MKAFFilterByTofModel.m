...
@interface MKAFFilterByTofModel ()
...
@implementation MKAFFilterByTofModel
...
- (BOOL)readFilterStatus {
    ...
    [MKAFInterface af_readFilterBXPTofStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByTofStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readCodeList {
    __block BOOL success = NO;
    [MKAFInterface af_readFilterBXPTofMfgCodeListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCodeList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterBXPTofList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
