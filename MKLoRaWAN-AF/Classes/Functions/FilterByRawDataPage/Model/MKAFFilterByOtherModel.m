...
@implementation MKAFFilterRawAdvDataModel
...
@interface MKAFFilterByOtherModel ()
...
@implementation MKAFFilterByOtherModel
...
- (BOOL)readFilterStatus {
    ...
    [MKAFInterface af_readFilterByOtherStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByOtherStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readRelationship {
    __block BOOL success = NO;
    [MKAFInterface af_readFilterByOtherRelationshipWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configRelationship:(NSInteger)relationship {
    __block BOOL success = NO;
    [MKAFInterface af_configFilterByOtherRelationship:relationship sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readConditions {
    __block BOOL success = NO;
    [MKAFInterface af_readFilterByOtherConditionsWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configConditions:(NSArray <MKFilterRawAdvDataModel *>*)list {
    ...
    for (MKFilterRawAdvDataModel *model in list) {
        MKAFFilterRawAdvDataModel *tempModel = [[MKAFFilterRawAdvDataModel alloc] init];
        [tempModel updateWithModel:model];
        [tempList addObject:tempModel];
    }
    [MKAFInterface af_configFilterByOtherConditions:tempList sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
