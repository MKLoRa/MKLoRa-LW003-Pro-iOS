...
@implementation MKAFScanAlwaysModel
...
- (BOOL)readReportInterval {
    ...
    [MKAFInterface af_readScanAlwaysOnPeriodicReportIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReportInterval {
    __block BOOL success = NO;
    [MKAFInterface af_configScanAlwaysOnPeriodicReportInterval:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
