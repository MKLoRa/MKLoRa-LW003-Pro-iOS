...
@interface MKAFOnOffSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFOnOffSettingsModel *dataModel;

@end

@implementation MKAFOnOffSettingsController

- (void)dealloc {
    NSLog(@"MKAFOnOffSettingsController销毁");
}
...
- (void)configOffByButton:(BOOL)isOn {
    ...
    [MKAFInterface af_configTurnOffDeviceByButtonStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configShutDownPayload:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKAFInterface af_configShutDownPayloadStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configAutoPowerOn:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKAFInterface af_configAutoPowerOnAfterCharging:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)commandPowerOff{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                              ...
                              isPenetration:NO];
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKAFInterface af_powerOffWithSucBlock:^{
        [[MKHudManager share] hide];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (MKAFOnOffSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFOnOffSettingsModel alloc] init];
    }
    return _dataModel;
}
