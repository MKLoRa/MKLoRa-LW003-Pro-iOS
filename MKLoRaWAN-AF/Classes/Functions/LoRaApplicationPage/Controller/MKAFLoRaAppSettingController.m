//
//  MKAFLoRaAppSettingController.m
...
@interface MKAFLoRaAppSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAFLoRaAppSettingModel *dataModel;

@end

@implementation MKAFLoRaAppSettingController

- (void)dealloc {
    NSLog(@"MKAFLoRaAppSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Multicase Group
        MKAFMulticaseGroupController *vc = [[MKAFMulticaseGroupController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Message Type Settings
        MKAFMessageTypeController *vc = [[MKAFMessageTypeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AF", @"MKAFLoRaAppSettingController", @"af_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAFLoRaAppSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAFLoRaAppSettingModel alloc] init];
    }
    return _dataModel;
}
