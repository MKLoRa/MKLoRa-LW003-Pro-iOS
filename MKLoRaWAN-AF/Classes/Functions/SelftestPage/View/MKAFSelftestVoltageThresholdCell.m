//
//  MKAFSelftestVoltageThresholdCell.m
...
@implementation MKAFSelftestVoltageThresholdCellModel
@end

@interface MKAFSelftestVoltageThresholdCell ()
...
@implementation MKAFSelftestVoltageThresholdCell

+ (MKAFSelftestVoltageThresholdCell *)initCellWithTableView:(UITableView *)tableView {
    MKAFSelftestVoltageThresholdCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAFSelftestVoltageThresholdCellIdenty"];
    if (!cell) {
        cell = [[MKAFSelftestVoltageThresholdCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAFSelftestVoltageThresholdCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.thresholdList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(af_selftestVoltageThresholdCell_thresholdChanged:threshold:)]) {
            [self.delegate af_selftestVoltageThresholdCell_thresholdChanged:self.dataModel.index threshold:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKAFSelftestVoltageThresholdCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAFSelftestVoltageThresholdCellModel.class] || _dataModel.threshold > 20) {
        return;
    }
