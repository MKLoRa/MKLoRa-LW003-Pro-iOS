...
/**
 线程ID
 */
@property (nonatomic, assign)mk_af_taskOperationID operationID;
...
- (instancetype)initOperationWithID:(mk_af_taskOperationID)operationID
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKAFTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKAFTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_af_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_af_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    //分帧数据
    NSString *totalNum = returnData[mk_af_totalNumKey];
    if (MKValidStr(totalNum)) {
        //存在分帧数据
        self.receiveTimerCount = 0;
        NSData *data = returnData[mk_af_contentKey];
        if (data) {
