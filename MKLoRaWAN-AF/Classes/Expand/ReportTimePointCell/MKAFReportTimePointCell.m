...
@implementation MKAFReportTimePointCellModel
@end

@interface MKAFReportTimePointCell ()
...
@implementation MKAFReportTimePointCell

+ (MKAFReportTimePointCell *)initCellWithTableView:(UITableView *)tableView {
    MKAFReportTimePointCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAFReportTimePointCellIdenty"];
    if (!cell) {
        cell = [[MKAFReportTimePointCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAFReportTimePointCellIdenty"];
    }
    return cell;
}
...
- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(af_cellDeleteButtonPressed:index:)]) {
        [self.delegate af_cellDeleteButtonPressed:self.dataModel.type index:self.dataModel.index];
    }
}
...
- (void)hourButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(af_hourButtonPressed:index:)]) {
        [self.delegate af_hourButtonPressed:self.dataModel.type index:self.dataModel.index];
    }
}
...
- (void)timeSpaceButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(af_timeSpaceButtonPressed:index:)]) {
        [self.delegate af_timeSpaceButtonPressed:self.dataModel.type index:self.dataModel.index];
    }
}

- (void)contentPanelTapAction {
    if (self.contentPanel.frame.origin.x == 0) {
        if ([self.delegate respondsToSelector:@selector(af_cellTapAction)]) {
            [self.delegate af_cellTapAction];
        }
        return;
    }
    if (self.contentPanel.frame.origin.x < 0){
...
- (void)swipeEventBeTriggered:(UISwipeGestureRecognizer *)swipeGesture{
    if ([self.delegate respondsToSelector:@selector(af_cellResetFrame)]) {
        [self.delegate af_cellResetFrame];
    }
...
#pragma mark - setter
- (void)setDataModel:(MKAFReportTimePointCellModel *)dataModel {
    _dataModel = nil;
