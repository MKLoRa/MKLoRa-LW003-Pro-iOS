...
@implementation MKAFFilterByRawDataCellModel
...
@interface MKAFFilterByRawDataCell ()
...
@implementation MKAFFilterByRawDataCell

+ (MKAFFilterByRawDataCell *)initCellWithTableView:(UITableView *)tableView {
    MKAFFilterByRawDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAFFilterByRawDataCellIdenty"];
    if (!cell) {
        cell = [[MKAFFilterByRawDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAFFilterByRawDataCellIdenty"];
    }
    return cell;
}
...
- (void)textFieldValueChanged:(NSString *)text textType:(mk_af_filterRawAdvDataTextType)type {
    if ([self.delegate respondsToSelector:@selector(af_rawFilterDataChanged:index:textValue:)]) {
        [self.delegate af_rawFilterDataChanged:type index:self.dataModel.index textValue:text];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKAFFilterByRawDataCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAFFilterByRawDataCellModel.class]) {
        return;
    }
...
- (MKTextField *)typeTextField {
    if (!_typeTextField) {
        ...
        _typeTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            [sself textFieldValueChanged:text textType:mk_af_filterRawAdvDataTextTypeDataType];
        };
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_af_filterRawAdvDataTextTypeMinIndex];
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_af_filterRawAdvDataTextTypeMaxIndex];
        };
    }
...
- (MKTextField *)rawDataField {
    if (!_rawDataField) {
        ...
        _rawDataField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_af_filterRawAdvDataTextTypeRawDataType];
        };
    }
