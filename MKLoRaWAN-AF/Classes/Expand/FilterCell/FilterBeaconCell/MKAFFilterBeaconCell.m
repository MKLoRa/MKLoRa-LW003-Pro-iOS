...
@implementation MKAFFilterBeaconCellModel
@end

@interface MKAFFilterBeaconCell ()
...
@implementation MKAFFilterBeaconCell

+ (MKAFFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKAFFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAFFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKAFFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAFFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKAFFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAFFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_af_beaconMinValueChanged:index:)]) {
                [self.delegate mk_af_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_af_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_af_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
